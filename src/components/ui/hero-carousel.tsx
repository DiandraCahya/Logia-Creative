      ...
      >
        {onBack ? (
          <button
            type="button"
            onClick={onBack}
            className="opacity-90 transition-opacity hover:opacity-100 flex items-center justify-center"
            ...
            >
              {active.meta.map((fact, i) => (
                <motion.span
                  ...
                  transition={
                    reduced ? { duration: 0 } : { type: "spring", stiffness: 100, damping: 20, delay: 0.1 + i * 0.05 }
                  }
                >
                  {fact}
                </motion.span>
